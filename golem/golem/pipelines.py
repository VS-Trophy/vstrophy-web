...
from .items import WeekItem, MatchItemVST, PlayerPerformanceItemVST, PlayerUpdateItemVST
...
class MatchVSTPipeline(ArangoPipeline):
    ...
    def insert_and_link_player_of_roster(self, roster_id, roster_spot):
        if roster_spot is not None:
            ...
            edge_id = self.get_edge_id(
                self.playedInVST.name, player_id, roster_id)
    ...
    def close_spider(self, spider):
        spider.logger.info(
            "Inserted " + str(self.match_insert_count) + " matches.")
        spider.logger.info(
            "Inserted " + str(self.roster_insert_count) + " rosters.")
        spider.logger.info(
            "Updated " + str(self.roster_update_count) + " rosters.")
        spider.logger.info(
            "Updated " + str(self.roster_spot_update_count) + " rosters spots.")
...
class PlayerPerformanceVSTPipeline(ArangoPipeline):
    ...
    def get_player_key_from_fdid(self, fdid):
        cursor = self.db.aql.execute('''
        FOR player IN players
            FILTER player.fdid == @fdid
            RETURN player._key
        ''', bind_vars={'fdid': int(fdid)},count=True)

        assert cursor.count() < 2, "Two players with fdid " + fdid
        return None if cursor.empty() else cursor.next()
    ...
    @check_pipeline
    def process_item(self, item, spider):
        player_key = self.get_player_key_from_fdid(item['PlayerID'])
        if player_key is None:
            spider.logger.warn("Player with fdid " + str(item['PlayerID']) + " (" + str(item["Name"])+")not found in database! Creating him.")
            playerDoc = {
                '_key': 'fdid:' + str(item['PlayerID']), #add a prefix 
                'name': item['Name'],
                'fdid':  item['PlayerID']
            }
            self.insert_if_not_present(self.players,playerDoc,silent=True)
            self.player_insert_count += 1
            return
        week_id='weeks/' + self.get_week_key(item['Season'], item['Week'])
        player_id='players/' + player_key
        itemDict=dict(item)
        itemDict['_key'] = str(itemDict['Season']) + '.' + str(itemDict['Week']) + '.' + player_key
        if self.get_edge_id('performedInWeek', player_id, week_id) is None:
            self.performed_in_week.link(player_id, week_id, data=itemDict)
            self.player_performance_insert_count += 1
        else:
            self.performed_in_week.update(itemDict, silent=True)
            self.player_performance_update_count += 1
        return item


class PlayerUpdateVSTPipeline(ArangoPipeline):
    itemclass = PlayerUpdateItemVST

    def __init__(self):
        super(PlayerUpdateVSTPipeline, self).__init__()
        self.player_update_count = 0

    def close_spider(self, spider):
        spider.logger.info(
            "Updated " + str(self.player_update_count) + " player birthdays.")

    @check_pipeline
    def process_item(self, item, spider):
        base_document={'_key': item['player_key']}
        if 'player_birthday' in item:
            base_document['birthday']=item['player_birthday']
        if 'player_fd_id' in item:
            base_document['fdid']=item['player_fd_id']
        self.players.update(base_document)
        self.player_update_count += 1
        return item
